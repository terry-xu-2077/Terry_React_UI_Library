...
export type PropertyRowProps={label:string;description?:string;changed?:boolean;children:ReactNode;onCopy?:()=>void;copyable?:boolean};
export function PropertyRow({label,description,changed,children,onCopy,copyable=false}:PropertyRowProps){const rowId=useId();const showCopy=copyable&&!!onCopy;return <div id={rowId} className={`tc-property-row ${changed?"is-changed":""}`}><div className="tc-property-label">{showCopy&&<button type="button" className="tc-copy" onClick={onCopy} aria-label="复制值" title="复制值"><Copy size={15}/></button>}<div><strong>{label}</strong>{description&&<span>{description}</span>}</div></div><div className="tc-property-value">{children}</div>{changed&&<span className="tc-changed">已修改</span>}</div>}
