...
export function PropertyRow({ label, description, changed, children, onCopy }: { label:string; description?:string; changed?:boolean; children:ReactNode; onCopy?:()=>void }) {
  const rowId = useId();
  return <div id={rowId} className={`tc-property-row ${changed?"is-changed":""}`}><div className="tc-property-label">{onCopy&&<button type="button" className="tc-copy" onClick={onCopy}>↩</button>}<div><strong>{label}</strong>{description&&<span>{description}</span>}</div></div><div className="tc-property-value">{children}</div>{changed&&<span className="tc-changed">已修改</span>}</div>;
}

export function Dialog({ open, title, children, onClose }: { open:boolean; title:string; children:ReactNode; onClose:()=>void }) {
  if (!open) return null;
  return <div className="tc-dialog-layer" onMouseDown={e=>{if(e.currentTarget===e.target) onClose()}}><section className="tc-dialog tc-dialog-in" role="dialog" aria-modal="true"><header><strong>{title}</strong><button type="button" onClick={onClose}><X size={18}/></button></header><div className="tc-dialog-body">{children}</div></section></div>;
}

export function Button({ children, onClick, disabled=false, className="" }: React.PropsWithChildren<{onClick?:()=>void;disabled?:boolean;className?:string}>) {
